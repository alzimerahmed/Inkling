...
class SpHighlightedText extends StatelessWidget {
  const SpHighlightedText({super.key, required this.text, required this.query, this.maxLines, this.style});
  ...
  Widget build(BuildContext context) {
    ...
    final highlightStyle =
        style?.copyWith(backgroundColor: ColorScheme.of(context).inversePrimary) ??
        TextStyle(backgroundColor: ColorScheme.of(context).inversePrimary);

    return Text.rich(
      TextSpan(
        children: [
          for (final segment in segments) TextSpan(text: segment.text, style: segment.isMatch ? highlightStyle : style),
        ],
      ),
      maxLines: maxLines,
