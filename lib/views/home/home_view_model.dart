...
class HomeViewModel extends ChangeNotifier with DisposeAwareMixin {
  ...
  /// Opens the editor seeded with today's journaling prompt as the page title
  /// and first line, via an ephemeral (never-persisted) template.
  Future<void> goToDailyPromptPage(BuildContext context) async {
    final DateTime now = DateTime.now();
    final String prompt = JournalingPromptsService.todayPrompt();

    final template = TemplateDbModel.newTemplate(
      createdAt: now,
      content: StoryContentDbModel(
