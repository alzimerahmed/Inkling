...
class GenerateBodyPlainTextResult {
  ...
  const GenerateBodyPlainTextResult({required this.plainText, required this.richPagesWithCounts});
}
  ...
  static GenerateBodyPlainTextResult? call(List<StoryPageDbModel>? newRichPages) {
    ...
    final String plainText = [
      ...
      if (newRichPages.length > 1)
        for (int i = 1; i < newRichPages.length; i++) ...[newRichPages[i].title ?? '', pageTexts[i]],
    ].join('\n').trim();
    ...
    return GenerateBodyPlainTextResult(plainText: plainText, richPagesWithCounts: richPagesWithCounts);
  }
}
