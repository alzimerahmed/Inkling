...
class BackupImagesUploaderService {
  ...
  Future<({bool uploaded, bool sourceMissing})> _uploadAsset(
    ...
    try {
      final cloudFile = await RetryExecutor.execute(
        () =>
            cloudService.uploadFile(cloudFileName, asset.localFile!, folderName: asset.type.subDirectory.relativePath),
      ...
      throw exp.AuthException(e.message, e.type, serviceType: cloudService.serviceType, context: e.context);
    } catch (e) {
  ...
  Future<List<AssetDbModel>> pendingAssets(
    ...
    return assets?.items
            ...
            .where((e) => e.localFile?.existsSync() == true || findBackfillSource(e, allServices, cloudService) != null)
            .toList() ??
        [];
  }
}
