...
abstract class BaseAnalyticsEventAdaptor {
  ...
  Future<void> logViewRoute({required BaseRoute routeObject, Map<String, String?>? analyticsParameters}) {
  ...
  Future<void> logViewSheet({required BaseBottomSheet bottomSheet}) {
    return logScreenView(screenClass: bottomSheet.analyticScreenClass, screenName: bottomSheet.analyticScreenName);
  }
  ...
  Future<void> logSaveStoryAsTemplate({required StoryDbModel story, required TemplateDbModel template}) {
  ...
  Future<void> logViewImages({required int imagesCount, int videosCount = 0}) {
    return logEvent(
      ...
      parameters: sanitizeParameters({'images_count': imagesCount.toString(), 'videos_count': videosCount.toString()}),
    );
  }
  ...
  Future<void> logUseGalleryTemplate({required String templateId, required String source}) {
    return logEvent(
  ...
  Map<String, Object>? tagAnalyticParameters(TagDbModel tag) =>
      sanitizeParameters({'category': tagCategoryLabel(tag), 'has_emoji': (tag.emoji != null).toString()});
