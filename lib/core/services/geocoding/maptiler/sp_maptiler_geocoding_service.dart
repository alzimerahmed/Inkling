...
class SpMapTilerGeocodingService implements SpGeocodingService {
  ...
  Future<PlaceDbModel?> reverseGeocode(SpLatLng latLng) async {
    try {
      ...
      final url = Uri.parse('$_baseUrl/${latLng.longitude},${latLng.latitude}.json?key=$kMapTilerApiKey');
      ...
      String? extractFromContext(String type) {
        final ctxItem = context.firstWhere((c) => (c['id'] as String).startsWith('$type.'), orElse: () => null);
  ...
  Future<List<PlaceDbModel>> searchPlaces(
    ...
    try {
      ...
      if (response.statusCode != 200) {
        AppLogger.error('SpMapTilerGeocodingService.searchPlaces API error: ${response.statusCode} - ${response.body}');
        return [];
      }
