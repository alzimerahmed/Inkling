...
void main() {
  group('BackupSyncStateStore', () {
    ...
    test(
      'onConnectionChecked sets connectionStatus only for the services in the map',
      () {
        ...
        expect(
          store.statusFor(BackupServiceType.google_drive).connectionStatus,
    ...
    test('onSyncQueueStarted marks every listed service queued', () {
      store.onSyncQueueStarted([
        BackupServiceType.google_drive,
        BackupServiceType.nextcloud,
      ]);

      expect(
        store.statusFor(BackupServiceType.google_drive).activity,
        SyncActivity.queued,
      );
      expect(
        store.statusFor(BackupServiceType.nextcloud).activity,
        SyncActivity.queued,
      );
    });

    test(
      'onServiceSyncStarted clears any leftover step/message from a previous run',
      () {
        store.onSyncMessage(
          BackupSyncMessage(
            processing: false,
    ...
    test(
      'onServiceSyncFinished without lastSyncedAt keeps the previous value',
      () {
        final syncedAt = DateTime(2026, 1, 1);
        store.onServiceSyncFinished(
          BackupServiceType.google_drive,
        ...
        store.onServiceSyncFinished(
          BackupServiceType.google_drive,
        ); // failed run, no new timestamp

        expect(
          store.statusFor(BackupServiceType.google_drive).lastSyncedAt,
          syncedAt,
        );
      },
    );
    ...
    test(
      'onSyncFinished clears any service left queued/active if the run ended early',
      () {
        store.onSyncQueueStarted([
          BackupServiceType.google_drive,
          BackupServiceType.nextcloud,
