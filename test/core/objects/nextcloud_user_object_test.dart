...
void main() {
  group('NextcloudUserObject', () {
    ...
    test(
      'identifier distinguishes two installs sharing a host under different paths',
      () {
    ...
    test(
      'identifier strips leading/trailing slashes from the deployment path',
      () {
        final pathed = NextcloudUserObject(
          serverUrl: 'https://example.com/cloud-a/',
          username: 'admin',
    ...
    test('authHeaders sends HTTP Basic auth with username:appPassword', () {
      ...
      final decoded = utf8.decode(
        base64Decode(headers['Authorization']!.replaceFirst('Basic ', '')),
      );
      expect(decoded, 'thea:app-password-token');
    });

    test(
      'folderName defaults to null for accounts connected before it existed',
      () {
    ...
    test(
      'destinationKey falls back to the default folder when folderName is null',
      () {
        expect(user.folderName, isNull);
        expect(
          user.destinationKey,
          '${user.identifier}/${NextcloudUserObject.defaultFolderName}',
        );
      },
    );

    test(
      'destinationKey folds in an explicit folder name matching the default',
      () {
        ...
        expect(
          explicitDefault.destinationKey,
          '${explicitDefault.identifier}/${NextcloudUserObject.defaultFolderName}',
        );
      },
    );

    test(
      'destinationKey folds in a custom folder name, percent-encoding its own slashes',
      () {
        ...
        expect(
          customFolder.destinationKey,
          '${customFolder.identifier}/Journals%2FMyDiary',
        );
      },
    );

    test(
      'destinationKey is unambiguous when a deployment path and a nested folder could otherwise collide',
      () {
        ...
        expect(
          deploymentPath.destinationKey,
          isNot(nestedFolder.destinationKey),
        );
      },
    );
    ...
    test('destinationKey differs between two accounts using different folders, '
      ...
      expect(
        folderA.destinationKey,
        isNot(folderB.destinationKey),
      ); // different storage location
    });

    test(
      'configuration shows the folder as a ~/-prefixed path — never the app password',
      () {
        final withFolder = NextcloudUserObject(
          serverUrl: 'https://cloud.example.com',
          username: 'thea',
        ...
        expect(
          withFolder.configuration.any(
            (e) => e.value.contains('super-secret-token'),
          ),
          isFalse,
        );
      },
    ...
    test(
      'configuration folder falls back to the default when folderName is null',
      () {
        expect(user.folderName, isNull);
        expect(
          user.configuration.single.value,
