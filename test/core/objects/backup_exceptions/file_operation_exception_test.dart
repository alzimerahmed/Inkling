...
void main() {
  group('FileOperationException', () {
    ...
    test('creates with correct properties for download operation', () {
      const exception = FileOperationException('Download failed', FileOperationType.download);
    ...
    test('creates with correct properties for delete operation', () {
      const exception = FileOperationException('Delete failed', FileOperationType.delete, context: 'old_backup.json');
    ...
    test('creates with correct properties for list operation', () {
      const exception = FileOperationException('List failed', FileOperationType.list, context: 'backup_folder');
    ...
    test('is retryable by default', () {
      const exception = FileOperationException('Operation failed', FileOperationType.upload);
      expect(exception.isRetryable, isTrue);
    });

    test('can be made non-retryable', () {
      const exception = FileOperationException('Operation failed', FileOperationType.upload, isRetryable: false);
      expect(exception.isRetryable, isFalse);
    });

    test('toString includes operation context', () {
      const exception = FileOperationException('Upload failed', FileOperationType.upload, context: 'test_file.json');
