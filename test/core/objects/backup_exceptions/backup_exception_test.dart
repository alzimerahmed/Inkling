...
class TestBackupException extends BackupException {
  const TestBackupException(super.message, {super.context, super.isRetryable = false, super.serviceType});
}

void main() {
  group('BackupException', () {
    test('creates with correct properties', () {
      const exception = TestBackupException('Test error', context: 'test_operation', isRetryable: true);
    ...
    test('toString includes context when provided', () {
      const exception = TestBackupException('Test error', context: 'test_context');
    ...
    test('can be made retryable', () {
      const exception = TestBackupException('Test error', isRetryable: true);
      expect(exception.isRetryable, isTrue);
    });
  });
}
