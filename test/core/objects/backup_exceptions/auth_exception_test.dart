...
void main() {
  group('AuthException', () {
    test('creates with correct properties for token expired', () {
      const exception = AuthException('Token expired', AuthExceptionType.tokenExpired, context: 'test_auth');
    ...
    test('creates with correct properties for insufficient scopes', () {
      const exception = AuthException('Insufficient scopes', AuthExceptionType.insufficientScopes);
    ...
    test('creates with correct properties for sign in required', () {
      const exception = AuthException('Sign in required', AuthExceptionType.signInRequired);
    ...
    test('creates with correct properties for sign in failed', () {
      const exception = AuthException('Sign in failed', AuthExceptionType.signInFailed);
    ...
    test('is not retryable by default', () {
      const exception = AuthException('Auth error', AuthExceptionType.tokenExpired);
      expect(exception.isRetryable, isFalse);
    });
