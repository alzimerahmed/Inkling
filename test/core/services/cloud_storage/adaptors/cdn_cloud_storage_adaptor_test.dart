  ...
  group('CdnCloudStorageAdaptor', () {
    ...
    group('downloadBytes', () {
      test('returns body bytes on HTTP 200', () async {
        ...
        final adaptor = _adapterWith(MockClient((_) async => http.Response.bytes(expected, 200)));
      ...
      test('throws CloudStorageUnauthorizedException on HTTP 401', () async {
        final adaptor = _adapterWith(MockClient((_) async => http.Response('Unauthorized', 401)));
        await expectLater(adaptor.downloadBytes('/secure.bin'), throwsA(isA<CloudStorageUnauthorizedException>()));
      });

      test('throws CloudStorageUnauthorizedException on HTTP 403', () async {
        final adaptor = _adapterWith(MockClient((_) async => http.Response('Forbidden', 403)));
        await expectLater(adaptor.downloadBytes('/private.bin'), throwsA(isA<CloudStorageUnauthorizedException>()));
      });

      test('returns null on non-200 non-401/403 status', () async {
        final adaptor = _adapterWith(MockClient((_) async => http.Response('Not Found', 404)));
        final bytes = await adaptor.downloadBytes('/missing.bin');
        expect(bytes, isNull);
      });

      test('returns null on network error', () async {
        final adaptor = _adapterWith(MockClient((_) async => throw Exception('Network error')));
        final bytes = await adaptor.downloadBytes('/file.bin');
        expect(bytes, isNull);
      });
    });
