...
void main() {
  group('PeriodPredictionService.predictNextPeriodStart', () {
    test('returns null without enough history', () {
      ...
      expect(PeriodPredictionService.predictNextPeriodStart([DateTime(2026, 1, 1)]), isNull);
    });
    ...
    test('predicts next start from two 28-day cycles', () {
      ...
      final next = PeriodPredictionService.predictNextPeriodStart(dates, now: DateTime(2026, 2, 1));
    ...
    test('rolls forward whole cycles when naive prediction is in the past', () {
      final dates = [DateTime(2026, 1, 1), DateTime(2026, 1, 29)];
      // avg cycle = 28. Jan 29 + 28 = Feb 26, which is before "now" (Apr 1),
      // so it rolls forward: Feb 26 -> Mar 26 -> Apr 23.
      final next = PeriodPredictionService.predictNextPeriodStart(dates, now: DateTime(2026, 4, 1));
    ...
    test('caps the averaging window to the most recent 6 months of starts', () {
      ...
      final next = PeriodPredictionService.predictNextPeriodStart(dates, now: DateTime(2026, 3, 1));
    ...
    test('groups a cycle that spans a month boundary as a single start', () {
      ...
      final next = PeriodPredictionService.predictNextPeriodStart(dates, now: DateTime(2026, 2, 3));
    ...
    test('returns null when the most recent start is older than the history window', () {
      ...
      final dates = [DateTime(2025, 1, 1), DateTime(2025, 1, 29)];
      final next = PeriodPredictionService.predictNextPeriodStart(dates, now: DateTime(2025, 9, 29));
      expect(next, isNull);
    });
    ...
    test('ignores duplicate day entries', () {
      final dates = [DateTime(2026, 1, 1), DateTime(2026, 1, 1), DateTime(2026, 1, 29)];
      final next = PeriodPredictionService.predictNextPeriodStart(dates, now: DateTime(2026, 2, 1));
      expect(next, equals(DateTime(2026, 2, 26)));
    });
  });
