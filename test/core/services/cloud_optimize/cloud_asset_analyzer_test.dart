...
void main() {
  group('CloudAssetAnalyzer', () {
    ...
    test('clean — file ID matches stored destination', () {
      ...
      final record = _asset(
        1001,
        cloudDestinations: _destinations('cloud-abc'),
      );
    ...
    test(
      'stale — record exists, file not attached, live copy IS in fetched set',
      () {
        ...
        final record = _asset(
          1002,
          cloudDestinations: _destinations('cloud-new'),
        );
    ...
    test('orphan guard — record exists, no live copy in fetched set → not stale', () {
      ...
      final record = _asset(
        1003,
        cloudDestinations: _destinations('cloud-missing'),
      );
    ...
    test("multi-account — each email's file is clean", () {
      ...
      expect(
        result.clean.map((f) => f.id),
        containsAll(['cloud-A', 'cloud-B']),
      );
      expect(result.stale, isEmpty);
      expect(result.detached, isEmpty);
    });
    ...
    test('3 files for same ID — 1 attached (clean), 2 stale', () {
      ...
      final record = _asset(
        3001,
        cloudDestinations: _destinations('cloud-live'),
      );
      ...
      expect(
        result.stale.map((f) => f.id),
        containsAll(['cloud-dup1', 'cloud-dup2']),
      );
      expect(result.detached, isEmpty);
    });
    ...
    test('all 3 files detached when no DB record exists', () {
      ...
      expect(
        result.detached.map((f) => f.id),
        containsAll(['cloud-a', 'cloud-b', 'cloud-c']),
      );
      expect(result.stale, isEmpty);
      expect(result.clean, isEmpty);
    });
    ...
    test('mixed batch — correct classification per asset', () {
      ...
      final cleanRecord = _asset(
        5001,
        cloudDestinations: _destinations('cf-5001'),
      );
      ...
      final staleRecord = _asset(
        5003,
        cloudDestinations: _destinations('cf-5003-live'),
      );
    ...
    test('null filename is excluded from all result lists', () {
      final nullNameFile = CloudFileObject(
        fileName: null,
        id: 'cloud-null',
        description: null,
      );
    ...
    test('counts are correct for total detached and stale', () {
      ...
      final record7001 = _asset(
        7001,
        cloudDestinations: _destinations('live-1'),
      );
    ...
    group('re-verification safety', () {
      test(
        'file promoted to attached between analysis and deletion → classified clean',
        () {
          ...
          final record = _asset(
            8001,
            cloudDestinations: _destinations('cloud-now-live'),
          );
      ...
      test(
        'live copy disappeared between analysis and deletion → orphan guard keeps file',
        () {
          ...
          final record = _asset(
            8002,
            cloudDestinations: _destinations('cloud-gone'),
          );
      ...
      test(
        'record deleted from DB between analysis and deletion → now detached',
        () {
    ...
    group('isDetachedEligibleForCleanup', () {
      ...
      test(
        'file old enough but tombstone deleted 5 days ago → not eligible',
        () {
          final file = fileWithAge(
            'f5',
            '5.jpg',
          ...
          final ts = tombstone(
            5,
            permanentlyDeletedAt: DateTime.now().subtract(
              const Duration(days: 5),
            ),
          );
          expect(
            CloudAssetAnalyzer.isDetachedEligibleForCleanup(
              file,
              tombstone: ts,
            ),
            isFalse,
      ...
      test('file old enough, tombstone deleted 31 days ago → eligible', () {
        ...
        expect(
          CloudAssetAnalyzer.isDetachedEligibleForCleanup(file, tombstone: ts),
          isTrue,
        );
      });
