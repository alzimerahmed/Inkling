...
AssetDbModel _asset(int id, {Map<String, Map<String, Map<String, String>>> cloudDestinations = const {}}) {
...
CloudAssetAnalysisResult _analyze(List<CloudFileObject> cloudFiles, Map<int, AssetDbModel> recordById) =>
    CloudAssetAnalyzer.analyze(cloudFiles: cloudFiles, recordById: recordById, serviceTypeId: _serviceId);
...
void main() {
  group('CloudAssetAnalyzer', () {
    ...
    test('all 3 files detached when no DB record exists', () {
      final files = [_file('cloud-a', '4001.jpg'), _file('cloud-b', '4001.jpg'), _file('cloud-c', '4001.jpg')];
    ...
    test('mixed batch — correct classification per asset', () {
      ...
      final result = _analyze([cleanFile, detachedFile, staleFile, liveFile], {5001: cleanRecord, 5003: staleRecord});
    ...
    group('isDetachedEligibleForCleanup', () {
      ...
      test('custom grace period is respected', () {
        ...
        expect(CloudAssetAnalyzer.isDetachedEligibleForCleanup(file, gracePeriod: const Duration(days: 7)), isTrue);
      });
    });
  });
}
