...
void main() {
  group('ReminderNavigationService.copyFor — custom reminders', () {
    test(
      'uses the user message as the title, with a fixed generic body',
      () async {
        final reminder = ReminderObject(
          id: 10,
          type: ReminderType.custom,
          message: 'Check ideas tag',
        );
    ...
    test(
      'throws if message is null — EditCustomReminderViewModel must never save one without it',
      () async {
        final reminder = ReminderObject(
          id: 13,
          type: ReminderType.custom,
        ...
        await expectLater(
          ReminderNavigationService.copyFor(reminder),
