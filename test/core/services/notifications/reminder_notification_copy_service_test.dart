...
void main() {
  group('ReminderNotificationCopyService content pools', () {
    ...
    Future<Set<(String, String)>> collect(
      Future<(String, String)> Function() pick, {
      int attempts = 200,
    }) async {
    ...
    test(
      'dailyCopy stays within a bounded, non-duplicated pool (<=20)',
      () async {
        ...
        expect(
          titles.length,
          seen.length,
          reason: 'no two pairs should share a title',
        );
      },
    );

    test(
      'onThisDayCopy stays within a bounded, non-duplicated pool (<=20)',
      () async {
    ...
    test(
      'periodCopy stays within a bounded, non-duplicated pool (<=20)',
      () async {
