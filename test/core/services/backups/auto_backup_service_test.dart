...
void main() {
  ...
  group('AutoBackupService.applyRetention', () {
    ...
    test('keeps the newest N files, deletes older ones', () async {
      ...
      final deleted = await AutoBackupService.applyRetention(directory: tempDir, keep: 2);
    ...
    test('keeps everything when under the limit', () async {
      ...
      final deleted = await AutoBackupService.applyRetention(directory: tempDir, keep: 5);
    ...
    test('keep=0 wipes the directory', () async {
      await createFile('a');
      final deleted = await AutoBackupService.applyRetention(directory: tempDir, keep: 0);
      expect(deleted, 1);
