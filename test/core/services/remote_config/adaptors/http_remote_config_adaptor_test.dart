...
HttpRemoteConfigAdaptor _adaptorWith(http.Client client) {
  return HttpRemoteConfigAdaptor(
...
void main() {
  ...
  group('HttpRemoteConfigAdaptor', () {
    test(
      'applies fetched values after the background refetch resolves',
      () async {
        final adaptor = _adaptorWith(
          MockClient(
            (_) async => http.Response(
              jsonEncode({'SURVEY_URL': 'https://example.com/survey'}),
              200,
            ),
          ),
        );
    ...
    test(
      'uses the disk cache immediately, before any network round trip',
      () async {
    ...
    test('getJsonString re-encodes a fetched nested object', () async {
      ...
      expect(jsonDecode(adaptor.getJsonString('FEATURE_FLAGS', '{}')), {
        'foo': true,
      });
    });

    test(
      'onConfigUpdated emits the fetched keys after a successful refetch',
      () async {
        final adaptor = _adaptorWith(
          MockClient(
            (_) async => http.Response(
    ...
    test(
      'getBool/getInt/getDouble coerce string values and fall back to defaults otherwise',
      () async {
        final adaptor = _adaptorWith(
          MockClient(
            (_) async => http.Response(
