...
class FakeWebdavTransport implements E2eSyncTransport {
  ...
  @override
  Future<bool> connect({required String serverUrl, required String username, required String appPassword}) async {
    return pingSucceeds;
  }
  ...
  Future<CloudFileObject?> uploadFile(String fileName, io.File file, {String? folderName}) async {
    ...
    return CloudFileObject(fileName: fileName, id: path, description: null, sizeInBytes: files[path]!.length);
  }
...
void main() {
  ...
  group('E2eSyncService', () {
    test('passphrase key derivation is deterministic across instances', () async {
    ...
    test('syncNow encrypts and uploads the backup artifact; remote bytes decrypt back to the original', () async {
      final transport = FakeWebdavTransport();
      final service = E2eSyncService(transport: transport, configStorage: TestE2eSyncConfigStorage());
      final config = testConfig();

      final io.Directory tempDir = await io.Directory.systemTemp.createTemp('inkling_test');
      final io.File backupFile = io.File('${tempDir.path}/backup.json.gz');
      final List<int> originalArchive = GzipService.compress('{"tables":{},"meta_data":{}}');
      await backupFile.writeAsBytes(originalArchive);

      try {
        final result = await service.syncNow(backupFile: backupFile, passphrase: 'passphrase', config: config);
        ...
        expect(uploaded.sublist(0, E2eEncryptionService.magicBytes.length), equals(E2eEncryptionService.magicBytes));

        // Decrypting the uploaded ciphertext restores the exact archive bytes.
        final Uint8List decrypted =
            await E2eEncryptionService().decrypt(encrypted: uploaded, passphrase: 'passphrase');
    ...
    test('syncNow refuses to run when sync is disabled (opt-in only)', () async {
      final transport = FakeWebdavTransport();
      final service = E2eSyncService(transport: transport, configStorage: TestE2eSyncConfigStorage());
      ...
      try {
        final result = await service.syncNow(backupFile: backupFile, passphrase: 'pw', config: testConfig(enabled: false));
    ...
    test('syncNow reports failure when the server is unreachable', () async {
      ...
      try {
        final result = await service.syncNow(backupFile: backupFile, passphrase: 'pw', config: testConfig());
        expect(result.success, isFalse);
        expect(transport.uploadCount, 0);
      } finally {
        tempDir.deleteSync(recursive: true);
    ...
    test('downloadDecryptAndRestore round-trips a payload uploaded by syncNow (decryption layer)', () async {
      final transport = FakeWebdavTransport();
      final config = testConfig();

      final List<int> archive = GzipService.compress('{"tables":{}}');
      final Uint8List encrypted = await E2eEncryptionService().encrypt(plaintext: archive, passphrase: 'pw');
      transport.files['/${E2eSyncConfigObject.defaultFolderName}/${E2eSyncService.defaultRemoteFileName}'] = encrypted;

      // Decrypt via the service's own path (restore itself needs a live DB,
      // so the DB merge is covered by integration tests).
      const String remotePath =
          '/${E2eSyncConfigObject.defaultFolderName}/${E2eSyncService.defaultRemoteFileName}';
      final List<int>? downloaded = await transport.downloadFileBytes(remotePath);
      expect(downloaded, isNotNull);

      final Uint8List decrypted = await E2eEncryptionService().decrypt(encrypted: downloaded!, passphrase: 'pw');
