...
void main() {
  group('GoogleDriveCloudService', () {
    test('creates NetworkException with correct properties', () {
      const exception = NetworkException('Connection failed', context: 'test_operation', isRetryable: true);
    ...
    test('creates AuthException for token expired', () {
      const exception = AuthException('Token expired', AuthExceptionType.tokenExpired, context: 'auth_test');
    ...
    test('creates AuthException for token revoked', () {
      const exception = AuthException('Token revoked', AuthExceptionType.tokenRevoked);
    ...
    test('creates FileOperationException for upload', () {
      const exception = FileOperationException('Upload failed', FileOperationType.upload, context: 'backup.json');
    ...
    test('creates ServiceException for data corruption', () {
      const exception = ServiceException('Data corrupted', ServiceExceptionType.dataCorrupted);
    ...
    test('handles sign-in cancellation', () {
      ...
      const exception = AuthException('Sign-in was cancelled by user', AuthExceptionType.signInFailed);
    ...
    test('handles missing access token', () {
      const exception = AuthException('Failed to obtain access token', AuthExceptionType.signInFailed);

      expect(exception.type, equals(AuthExceptionType.signInFailed));
    });

    test('handles insufficient scopes', () {
      const exception = AuthException('Insufficient scopes granted', AuthExceptionType.insufficientScopes);
